...
export default function App() {
  ...
  const [theme, setTheme] = useState<'light' | 'dark' | 'eye'>(() => {
  ...
  return (
    <div className="flex h-screen bg-editorial-paper text-editorial-ink selection:bg-editorial-accent/30 overflow-hidden font-sans">
      {!isDeepFocusActive && (
        <Sidebar 
      ...
      <div className={isDeepFocusActive ? "flex-1 flex flex-col h-full overflow-hidden" : "flex-1 flex flex-col h-full overflow-hidden pb-20 lg:pb-0"}>
        {!isDeepFocusActive && (
          <Header 
            currentView={currentView}
            onViewChange={setCurrentView}
