...
const AIChatView: React.FC = () => {
  ...
  return (
    <main className="h-full flex flex-col max-w-4xl mx-auto p-4 md:p-8 space-y-4 md:space-y-6">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-8 min-h-0">
            ...
            >
              ...
              <div className="space-y-1">
                 <div className={cn(
                   ...
                   message.role === 'user' 
                     ? "bg-editorial-paper border-editorial-ink/10 font-serif text-base md:text-lg text-editorial-ink" 
                     : "bg-white border-brand-blue/10 font-serif text-base md:text-lg text-editorial-ink shadow-sm"
                 )}>
                   {message.content}
                 </div>
        ...
        {loading && (
          <div className="flex space-x-4 items-center animate-pulse mr-auto">
      ...
      <div className="pt-4 border-t border-editorial-border sticky bottom-0 bg-editorial-paper/80 backdrop-blur-md">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
           {['Prioritize my day', 'Roadmap check', 'Organization help', 'Growth mindset'].map(suggestion => (
             <button
               key={suggestion}
               onClick={() => {
                 setInput(suggestion);
                 // Trigger focus or auto-send? Let's just set the input
               }}
        ...
        <form onSubmit={handleSend} className="relative group">
          ...
          <button 
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 bg-editorial-ink text-white rounded-sm hover:bg-brand-blue transition-all disabled:opacity-30 flex items-center justify-center"
          >
