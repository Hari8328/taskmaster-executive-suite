...
export default function AdminConsoleView() {
  ...
  const fetchTasks = async () => {
    ...
    try {
      const data = await taskService.getAllTasksAdmin(0, 1e3);
  ...
  const handleUserClick = (user) => {
    ...
    const filtered = tasks.filter((t) => t.User?.id === user.id);
  ...
  const filteredUsers = users.filter(
    (u) => u.username?.toLowerCase().includes(searchQuery.toLowerCase()) || u.displayName?.toLowerCase().includes(searchQuery.toLowerCase()) || u.email?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  const filteredTasks = tasks.filter(
    (t) => t.title?.toLowerCase().includes(searchQuery.toLowerCase()) || t.category?.toLowerCase().includes(searchQuery.toLowerCase()) || t.User?.username?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  return <div className="p-6 md:p-10 space-y-8 min-h-screen bg-editorial-paper text-editorial-ink pb-32">
      {
    /* Title */
  }
    ...
    className={`px-6 py-2.5 rounded-xl text-xs uppercase tracking-widest font-bold transition-all ${activeTab === "users" ? "bg-editorial-ink text-white shadow-sm" : "text-editorial-muted hover:bg-editorial-ink/5"}`}
  >
    ...
    className={`px-6 py-2.5 rounded-xl text-xs uppercase tracking-widest font-bold transition-all ${activeTab === "tasks" ? "bg-editorial-ink text-white shadow-sm" : "text-editorial-muted hover:bg-editorial-ink/5"}`}
  >
  ...
  />
    ...
    /* Content */
      ...
      {activeTab === "users" ? <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          {
    /* Users List Column */
  }
          <div className="lg:col-span-2 space-y-4">
            ...
            {loadingUsers ? <p className="text-sm font-mono text-editorial-muted py-8">Auditing user records...</p> : filteredUsers.length === 0 ? <p className="text-sm font-mono text-editorial-muted py-8">No users matched search criteria.</p> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredUsers.map((u) => <motion.div
    key={u.id}
    layoutId={`user-card-${u.id}`}
    onClick={() => handleUserClick(u)}
    className={`p-5 rounded-2xl border transition-all cursor-pointer text-left hover:shadow-md ${selectedUser?.id === u.id ? "border-brand-blue bg-brand-blue/[0.02] shadow-sm" : "border-editorial-accent/10 hover:border-editorial-accent/25"}`}
  >
                    <div className="flex items-center space-x-3">
                      {u.avatarUrl ? <img src={u.avatarUrl} alt="Avatar" className="w-10 h-10 rounded-xl object-cover" /> : <div className="w-10 h-10 rounded-xl bg-editorial-ink/5 flex items-center justify-center text-editorial-muted">
                          <UserIcon size={18} />
                        </div>}
                      <div>
    ...
    /* User Tasks Drawer Column */
          ...
          <div ref={drawerRef} className="bg-editorial-ink/[0.01] border border-editorial-accent/10 rounded-3xl p-6 space-y-6">
            {selectedUser ? <>
                ...
                <div className="space-y-3">
                  ...
                  {userTasks.length === 0 ? <p className="text-xs font-mono text-editorial-muted italic py-4">No tasks found for this user.</p> : <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-1">
                      {userTasks.map((t) => <div key={t.id} className="bg-white p-3 rounded-xl border border-editorial-accent/5 flex items-center justify-between text-left">
                          ...
                          <span className={`text-[9px] font-mono uppercase px-2 py-0.5 rounded-full shrink-0 ${t.status === "COMPLETED" ? "bg-editorial-ink/5 text-editorial-ink/50" : t.status === "IN_PROGRESS" || t.status === "IN-PROGRESS" ? "bg-brand-blue/10 text-brand-blue" : "bg-editorial-accent/10 text-editorial-ink"}`}>
                            {t.status}
                          </span>
                        </div>)}
                    </div>}
    ...
    <div className="space-y-4">
          ...
          {loadingTasks ? <p className="text-sm font-mono text-editorial-muted py-8">Scanning master databases...</p> : filteredTasks.length === 0 ? <p className="text-sm font-mono text-editorial-muted py-8">No tasks matched search query.</p> : <>
              {
      /* Mobile Cards View */
    }
              <div className="md:hidden space-y-3">
                {filteredTasks.map((t) => <div key={t.id} className="bg-white p-4 border border-editorial-accent/10 rounded-2xl space-y-3 text-left">
                    <div className="flex items-start justify-between">
                      <div className="truncate pr-2">
                        <span className="block text-sm font-bold text-editorial-ink truncate">{t.title}</span>
                        <span className="text-[10px] text-editorial-muted font-mono">{t.category || "General"}</span>
                      </div>
                      <span className={`text-[9px] font-mono uppercase px-2 py-0.5 rounded-full shrink-0 ${t.status === "COMPLETED" ? "bg-editorial-ink/5 text-editorial-ink/50" : t.status === "IN_PROGRESS" || t.status === "IN-PROGRESS" ? "bg-brand-blue/10 text-brand-blue" : "bg-editorial-accent/10 text-editorial-ink"}`}>
                        {t.status}
                    ...
                    <div className="flex items-center justify-between border-t border-editorial-accent/5 pt-3 text-xs">
                      <span className="text-[10px] font-mono text-editorial-muted">
                        Owner: {t.User ? <strong className="text-brand-blue">@{t.User.username}</strong> : "Unassigned"}
                      </span>
                      <span className={`font-mono text-[9px] uppercase px-2 py-0.5 rounded-full ${t.priority === "HIGH" ? "bg-red-50 text-red-600 border border-red-100" : t.priority === "MEDIUM" ? "bg-blue-50 text-brand-blue border border-blue-100" : "bg-slate-50 text-slate-600 border border-slate-100"}`}>
                        {t.priority}
                      </span>
                    </div>
                  </div>)}
      ...
      /* Desktop Table View */
              ...
              <div className="hidden md:block overflow-x-auto border border-editorial-accent/10 rounded-2xl bg-white shadow-sm">
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-editorial-accent/5">
                    {filteredTasks.map((t) => <tr key={t.id} className="hover:bg-editorial-ink/[0.01] transition-colors">
                        ...
                        <td className="py-3 px-6 font-mono text-editorial-ink">
                          {t.User ? <span className="font-bold text-brand-blue">@{t.User.username}</span> : <span className="text-editorial-muted">Unassigned</span>}
                        </td>
                        ...
                        <td className="py-3 px-6">
                          <span className={`font-mono text-[9px] uppercase px-2 py-0.5 rounded-full ${t.status === "COMPLETED" ? "bg-editorial-ink/5 text-editorial-ink/50" : t.status === "IN_PROGRESS" || t.status === "IN-PROGRESS" ? "bg-brand-blue/10 text-brand-blue" : "bg-editorial-accent/10 text-editorial-ink"}`}>
                            {t.status}
                          </span>
                        </td>
