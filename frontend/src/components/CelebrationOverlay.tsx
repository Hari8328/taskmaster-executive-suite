...
import { Trophy, Clock, Sparkles, X, Award, CheckCircle2 } from 'lucide-react';
import { TRIGGER_CELEBRATION_EVENT, CelebrationPayload, Achievement } from '../lib/audioAndAchievements';
...
const CelebrationOverlay: React.FC = () => {
  ...
  // Reset img error on new celebration
  useEffect(() => {
    if (show) {
      setImgError(false);
    }
  ...
  useEffect(() => {
    const handleCelebration = (event: Event) => {
      ...
      if (customEvent.detail) {
        ...
        // Auto-dismiss after 15 seconds so they can read the quote, question, achievements and story
  ...
  const formatSecsToHuman = (ms: number) => {
  ...
  return (
    <AnimatePresence>
      {show && activeCelebration && (
        <div className="fixed inset-x-0 bottom-6 md:bottom-10 z-50 flex flex-col items-center justify-end px-4 pointer-events-none">
          <motion.div
          ...
          >
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="space-y-1 flex-1 pr-6">
                ...
                {activeCelebration.durationMs ? (
                  <div className="flex items-center space-x-1 px-2 py-1 bg-brand-blue/5 border border-brand-blue/10 rounded w-fit mt-2 text-brand-blue text-xs font-mono">
                    <Clock size={12} className="shrink-0" />
                    <span>Focus Invested: {formatSecsToHuman(activeCelebration.durationMs)}</span>
                  </div>
                ) : (
                  <p className="text-[10px] font-serif italic text-editorial-muted mt-1">Focus logged immediately on completion.</p>
                )}
              </div>
            </div>

            {/* Inspirational Mentor segment */}
            {activeCelebration.mentor && (
              <div className="mt-5 pt-4 border-t border-editorial-border space-y-4">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-full overflow-hidden border border-editorial-muted/20 shrink-0 bg-editorial-muted/10 flex items-center justify-center">
                    {imgError ? (
                      <span className="font-mono font-bold text-xs text-editorial-ink">
                ...
                <div className="space-y-1.5">
                  ...
                  <ul className="list-disc pl-4 space-y-1 text-[11px] text-editorial-muted">
                    {activeCelebration.mentor.achievements.map((ach, idx) => (
                      <li key={idx} className="font-sans leading-relaxed">{ach}</li>
                    ))}
                  </ul>
                </div>
            ...
            {activeCelebration.newAchievements.length > 0 && (
              <motion.div 
              ...
              >
                ...
                {activeCelebration.newAchievements.map((ach) => (
                  <motion.div
                    key={ach.id}
