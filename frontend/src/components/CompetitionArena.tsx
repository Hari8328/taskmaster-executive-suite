...
import { Trophy, Crown, Sparkles, Swords, Zap, Award, Flame, User, Play, RefreshCw } from 'lucide-react';
...
export default function CompetitionArena({ userCompletionCount, userProfile }: CompetitionArenaProps) {
  // Initial default competitors (historical/inspiration figures)
  ...
  // Build the complete list containing the real user + other competitors
  const allParticipants: Competitor[] = [
    {
      id: 'current_user',
  ...
  // Sort them: Completed tasks descending. If tied, current user gets preferred priority, then alphabetical
  ...
  // Calculate high-scores and badges
  const maxCompleted = Math.max(...allParticipants.map(p => p.completedTasks));

  // Simulates an opponent completing a random task with an animation effect!
  ...
  return (
    <section className="border border-editorial-border p-6 md:p-8 bg-white shadow-sm relative overflow-hidden">
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  participant.isCurrentUser 
                    ...
                    : isFirst 
                    ? "border-amber-500/30" 
                    : "border-editorial-border/60 hover:border-editorial-border"
                )}
                ...
                <div className="flex items-center space-x-3.5 shrink-0">
                  ...
                  <div className="w-8 h-8 flex items-center justify-center shrink-0">
                    {isFirst ? (
                      ...
                      <span className="font-mono text-xs font-bold text-editorial-muted">
                        {position}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="relative shrink-0">
                    <div className={cn(
                      ...
                      participant.isCurrentUser 
                        ...
                        : "border-editorial-border bg-editorial-paper/50"
                    )}>
                      {participant.avatar && !imgErrors[participant.id] ? (
                        <img 
                    ...
                    {participant.isCurrentUser && (
                      <span className="absolute bottom-0 right-0 w-3 h-3 bg-brand-blue rounded-full border-2 border-white flex items-center justify-center">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      {participant.isCurrentUser && (
                        <span className="font-mono text-[8px] bg-brand-blue/10 text-brand-blue border border-brand-blue/20 rounded px-1 py-0.5 font-bold uppercase tracking-wider">
                          YOU
                        </span>
                      )}
                    </div>
      ...
      <div className="mt-5 p-3.5 bg-editorial-muted/5 border border-editorial-border/60 flex items-center space-x-3">
        ...
        <p className="text-xs text-editorial-muted font-serif italic">
          {sortedParticipants[0].isCurrentUser ? (
            <span>Highly exceptional! You are maintaining <strong>1st Position</strong> with {userCompletionCount} tasks completed. Protect your heroic legacy!</span>
          ) : (
            <span>You need <strong>{sortedParticipants[0].completedTasks - userCompletionCount + 1}</strong> more completed tasks to overtake <strong>{sortedParticipants[0].name}</strong> for 1st Position! Let's get to work!</span>
          )}
        </p>
      </div>
    </section>
