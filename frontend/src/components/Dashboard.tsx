...
import { getAIProgressCoaching, suggestPriority, getProductivitySuggestions, suggestBatchPriorities, hasGeminiKey, getCombinedDashboardInsight } from '../lib/gemini';
import { Sparkles, MoreHorizontal, Clock, ArrowUpRight, Loader2, CheckCircle2, Circle, Filter, X, PlayCircle, Bell, ChevronDown, Trash2, AlertTriangle, AlarmClock, History, Trophy, TrendingUp, TrendingDown } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ refreshTrigger, onViewChange }) => {
  ...
  const handleCompleteDeepFocusTask = async (task: Task, secondsFocused: number) => {
    try {
      const now = new Date();
      const secondsSpent = secondsFocused;
      const startTimeObj = new Date(now.getTime() - secondsSpent * 1000);
      
      setCompletingId(task.id);

      // Optimistically celebrate
      const updatedTasksList = tasks.map(t => t.id === task.id ? { ...task, status: 'completed' as Status } : t);
      const newUnlocks = checkAndUnlockAchievements(updatedTasksList);

      dispatchCelebration({
        taskTitle: task.title,
        durationMs: secondsSpent > 0 ? secondsSpent * 1000 : null,
  ...
  // Sync achievements when tasks load
  useEffect(() => {
    setAchievements(getAchievements());
  }, [tasks]);
  ...
  const fetchAIInsights = async (statsData: any, focusTimeStr: string, tasksList: any[]) => {
    ...
    try {
      ...
      // Fallback is handled inside the service usually, but if it throws:
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      let response;
      if (user && user.roles.includes('ROLE_ADMIN')) {
        response = await taskService.getAllTasksAdmin();
      } else {
        response = await taskService.getMyTasks();
      }
      // @ts-ignore - Task type in frontend might be slightly different from Backend Task entity
      const content = response.content || [];
      setTasks(content);
      
      // Calculate initial stats for AI coach
      const initialStats = {
        completed: content.filter((t: any) => t.status?.toLowerCase() === 'completed').length,
        inProgress: content.filter((t: any) => t.status?.toLowerCase() === 'in-progress').length,
        upcoming: content.filter((t: any) => t.status?.toLowerCase() === 'todo').length
      };
      
      const totalTimeMs = content
        .filter((t: any) => t.status?.toLowerCase() === 'completed' && t.startedAt && t.completedAt)
  ...
  const handleComplete = async (e: React.MouseEvent, task: Task) => {
    ...
    try {
      ...
      // Update local achievements on state
      setAchievements(getAchievements());
      ...
      // Delay fetch slightly to show animation
      setTimeout(fetchTasks, 1000);
    } catch (err) {
  ...
  const handleAiPriorityQuickSuggest = async (e: React.MouseEvent, task: Task) => {
    e.stopPropagation();
  ...
  const handleBatchAiPrioritize = async () => {
    ...
    try {
      ...
      // Update each task that has a different priority
  ...
  useEffect(() => {
    const timer = setInterval(() => setCurrentDate(new Date()), 1000 * 60); // Refresh every minute
  ...
  const totalFocusTimeMs = safeTasks
    .filter(t => t.status?.toLowerCase() === 'completed' && t.startedAt && t.completedAt)
    .reduce((acc, t) => {
      const s = new Date(t.startedAt!).getTime();
      const e = new Date(t.completedAt!).getTime();
      return acc + Math.max(0, e - s);
  ...
  // --- Consistency and Growth index calculations ---
  ...
  if (sortedDates.length > 0) {
    ...
    if (diffDays <= 1) {
      ...
      for (let i = 1; i < sortedDates.length; i++) {
        ...
        const dayDiff = Math.round((currentCheck.getTime() - prevDate.getTime()) / (1000 * 60 * 60 * 24));
        if (dayDiff === 1) {
          streak++;
  ...
  // Consistent ratio: completed high & medium priority tasks vs total high & medium
  ...
  const onTimeRatioValue = highMediumTotal > 0 ? highMediumCompleted / highMediumTotal : 1.0;
  const consistencyScore = Math.min(100, Math.round(onTimeRatioValue * 100));

  // Growth Index: completions this week vs previous week
  ...
  // Average completed time spent (average duration of tasks where we tracked start and completed time)
  const completedTasksWithDuration = safeTasks.filter(
    t => t.status?.toLowerCase() === 'completed' && t.startedAt && t.completedAt
  );

  const avgTimeToCompleteMs = completedTasksWithDuration.length > 0
    ? completedTasksWithDuration.reduce((acc, t) => {
        const s = new Date(t.startedAt!).getTime();
        const e = new Date(t.completedAt!).getTime();
        return acc + Math.max(0, e - s);
  ...
  return (
    <main className="p-4 md:p-8 space-y-12 pb-32 overflow-x-hidden">
      <TaskModal 
        task={selectedTask} 
        isOpen={isModalOpen} 
          ...
          <div className="max-w-2xl">
            {isAiCoachLoading ? (
              <div className="flex items-center space-x-2 text-editorial-muted">
                ...
                <div className="flex items-center space-x-2 text-red-600/70">
                  <AlertTriangle size={14} />
                  <p className="font-serif italic text-sm">{aiError}</p>
                </div>
            ) : aiCoachMessage.length > 0 ? (
              <motion.p 
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                  ...
                  <p className="font-serif italic text-editorial-muted">Synchronize your tasks to receive architectural guidance.</p>
                )}
              </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex items-start space-x-5">
              ...
              <div className="relative shrink-0">
                <div className="w-16 h-16 md:w-20 md:h-20 bg-gradient-to-tr from-[#1E293B] to-[#334155] border-2 border-brand-blue shadow-lg rounded-full flex items-center justify-center text-2xl md:text-3xl font-serif font-bold text-white uppercase select-none group-hover:scale-105 transition-transform duration-300">
                  {profile?.avatarUrl ? (
                    <img src={profile.avatarUrl} alt="Avatar" className="w-full h-full rounded-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
                    <span>{profile?.displayName ? profile.displayName[0] : (profile?.username ? profile.username[0] : 'H')}</span>
                  )}
                </div>
              ...
              <div className="space-y-1.5 py-1">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {isAdmin && (
                    <span className="px-2 py-0.5 bg-red-500/20 border border-red-500/30 text-red-300 rounded-full text-[9px] font-mono uppercase tracking-[0.1em]">
                      HQ Administrator
                    </span>
                  )}
                </div>
            ...
            <div className="bg-editorial-paper/5 border border-editorial-paper/10 p-5 rounded-xl space-y-3">
              ...
              <div className="flex items-center justify-between text-[10px] font-serif italic text-editorial-paper/60">
                ...
                {completedCount >= 10 ? (
                  <span className="text-emerald-400">At Ultimate rank ceiling! 👑</span>
                ) : (
                  <span>
            ...
            <div className="space-y-1 mt-2">
              <p className="text-base font-serif italic text-white leading-tight">
                {completionPercentage === 100 
                  ? "Flawless execution. Your schedule is perfectly completed! 🏆" 
                  : completionPercentage >= 75
                  ? "Superb! Highly optimized focus sessions today."
                  : completionPercentage >= 50 
                  ? "Past the median threshold. Excellent momentum."
                  : "Groundwork is set. Accelerate your day!"}
              </p>
          ...
          <div className="border border-editorial-border p-6 space-y-3 relative overflow-hidden flex flex-col justify-between">
            ...
            <div className="space-y-1.5">
              <div className="flex flex-wrap gap-1.5 items-center">
                {growthIndex >= 0 ? (
                  <span className="inline-flex items-center px-1.5 py-0.5 bg-green-50 border border-green-200 text-green-700 text-[10px] font-mono rounded">
                    <TrendingUp size={10} className="mr-1" />
                    +{growthIndex}% Growth
                  </span>
                ) : (
                  <span className="inline-flex items-center px-1.5 py-0.5 bg-red-50 border border-red-200 text-red-700 text-[10px] font-mono rounded">
                    <TrendingDown size={10} className="mr-1" />
                    {growthIndex}% Trend
                  </span>
                )}
                {streak > 0 && (
                  <span className="inline-flex items-center px-1.5 py-0.5 bg-orange-50 border border-orange-200 text-orange-700 text-[10px] font-mono rounded">
                    🔥 {streak}d Streak
                  </span>
                )}
              </div>
          ...
          {achievements.map((ach) => (
            <div 
              ...
              className={cn(
                ...
                ach.unlocked 
                  ? `${ach.badgeColor} shadow-md border-editorial-ink scale-100` 
                  : "border-dashed border-editorial-border bg-editorial-muted/5 opacity-40 filter grayscale hover:opacity-60"
              )}
            >
              ...
              {ach.unlocked && (
                <div className="absolute top-2 right-2 w-2.5 h-2.5 bg-green-500 rounded-full border border-white shadow shadow-green-500/50" title="Unlocked!" />
              )}
            </div>
          ))}
        </div>
      ...
      <section>
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 space-y-4 border border-dashed border-editorial-border">
            ...
            <div className="text-center py-20 border border-dashed border-editorial-border">
                    ...
                    >
                      ...
                      <div className="flex items-start space-x-4 md:space-x-6 w-full overflow-hidden">
                        ...
                        <AnimatePresence mode="wait">
                          {completingId === task.id || task.status?.toUpperCase() === 'COMPLETED' ? (
                            <motion.div
                              key="checked"
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          {task.User && (
                            <div className="px-2 py-0.5 bg-editorial-muted/10 text-editorial-muted text-[9px] font-mono uppercase tracking-wider rounded font-semibold">
                          ...
                          <div className={cn(
                            ...
                            task.priority?.toLowerCase() === 'medium' ? "bg-editorial-accent" : "bg-editorial-muted"
                          ...
                          <div className="relative overflow-hidden flex flex-wrap items-center gap-2">
                            <motion.h4 
                              className={cn(
                            ...
                            {(completingId === task.id || task.status?.toLowerCase() === 'completed') && (
                              <motion.div 
                                initial={{ width: 0 }}
                                animate={{ width: "100%" }}
                        ...
                        <div className="flex flex-wrap items-center gap-3 md:gap-5 mt-4">
                          ...
                          {task.reminderMinutes ? (
                            <div className="flex items-center space-x-1 text-editorial-accent">
                          ...
                          {task.status?.toLowerCase() !== 'completed' && task.dueDate && (
                            <div className={cn(
                          ...
                          <div className="flex-1 max-w-[140px] flex flex-col space-y-1">
                            <div className="flex justify-between items-center text-[9px] font-mono uppercase text-editorial-muted font-semibold">
                              <span>Progress</span>
                              <span>
                                {task.status?.toLowerCase() === 'completed' || completingId === task.id ? '100%' : 
                                 task.status?.toLowerCase() === 'in-progress' ? '60%' : '15%'}
                              </span>
                            </div>
                            <div className="h-1.5 bg-editorial-border w-full overflow-hidden rounded-full font-semibold">
                              <motion.div 
                                initial={{ width: 0 }}
                                animate={{ 
                                  width: (task.status?.toLowerCase() === 'completed' || completingId === task.id) ? '100%' : 
                                         task.status?.toLowerCase() === 'in-progress' ? '60%' : '15%' 
                                }}
                                transition={{ 
                                  type: "spring",
                                ...
                                className={cn(
                                  "h-full rounded-full",
                                  (task.status?.toLowerCase() === 'completed' || completingId === task.id) ? "bg-green-500" : 
                                  task.status?.toLowerCase() === 'in-progress' ? "bg-editorial-accent" : "bg-editorial-muted"
                                )}
                              />
                            </div>
                          </div>
      ...
      <section className="mt-16">
        ...
        <div className="border border-editorial-border overflow-hidden">
          ...
          <div className="divide-y divide-editorial-border">
            ...
            {safeTasks.filter(t => t.status?.toLowerCase() === 'completed').length === 0 && (
