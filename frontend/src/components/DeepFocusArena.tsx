...
import { soundscapeSynth, SoundscapeType } from '../lib/soundscapes';
import { playCompleteSound } from '../lib/audioAndAchievements';

interface DeepFocusArenaProps {
  onExit: () => void;
  initialTask: Task | null;
  tasks: Task[];
  onCompleteTask: (task: Task, secondsSpent: number) => Promise<void>;
}

export default function DeepFocusArena({ onExit, initialTask, tasks, onCompleteTask }: DeepFocusArenaProps) {
  const [activeTask, setActiveTask] = useState<Task | null>(initialTask);
  const [customGoal, setCustomGoal] = useState<string>('');
  const [isTaskDropdownOpen, setIsTaskDropdownOpen] = useState(false);

  // Timer States
  const [timerMode, setTimerMode] = useState<'pomodoro' | 'short' | 'long' | 'stopwatch'>('pomodoro');
  const [secondsRemaining, setSecondsRemaining] = useState(1500); // 25 min default
  const [isTimerRunning, setIsTimerRunning] = useState(false);
  const [secondsFocused, setSecondsFocused] = useState(0); // actual tracked seconds spent focusing

  const secondsFocusedRef = useRef(0);
  const activeTaskRef = useRef<Task | null>(null);
  const customGoalRef = useRef('');
  const timerModeRef = useRef<'pomodoro' | 'short' | 'long' | 'stopwatch'>('pomodoro');
  ...
  // Soundscape States
  const [soundType, setSoundType] = useState<SoundscapeType>('rain');
  const [soundVolume, setSoundVolume] = useState<number>(() => {
    return Number(localStorage.getItem('focus_sound_volume') || '0.5');
  });
  const [isSoundPlaying, setIsSoundPlaying] = useState(false);

  // Box Breathing Guide States (4s Inhale, 4s Hold, 4s Exhale, 4s Hold = 16s cycle)
  ...
  // Sync Timer Preset on Mode Selection
  useEffect(() => {
    ...
    else setSecondsRemaining(0); // stopwatch restarts at 0
  }, [timerMode]);
  ...
  const formatTimerDigits = (secCount: number) => {
  ...
  // Progress percentage for standard countdown types
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950 text-slate-100 flex flex-col justify-between p-6 md:p-10 select-none overflow-y-auto font-sans">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        ...
        {isSoundPlaying && (
          <div className="absolute inset-0 bg-[radial-gradient(#ffffff03_1px,transparent_1px)] bg-[size:24px_24px] opacity-60 animate-in fade-in duration-1000" />
        )}
      </div>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 items-center justify-center my-8 relative z-10">
        ...
        <section className="lg:col-span-5 space-y-8 flex flex-col justify-center h-full">
          ...
          {secondsFocused > 0 && (
            <div className="flex items-center space-x-2 text-[11px] font-mono text-slate-400 pl-1">
        ...
        <section className="lg:col-span-7 flex flex-col items-center justify-center space-y-8">
          ...
          <div className="flex items-center bg-slate-900 p-1 border border-slate-800 rounded-full max-w-md w-full justify-around text-xs font-mono">
            {(['pomodoro', 'short', 'long', 'stopwatch'] as const).map(mode => (
              <button
                key={mode}
                onClick={() => setTimerMode(mode)}
                className={`flex-1 text-center py-2 px-3 rounded-full uppercase tracking-wider font-bold transition-all text-[10px] md:text-xs ${
                  timerMode === mode 
          ...
          <div className="relative w-72 h-72 md:w-80 md:h-80 flex items-center justify-center bg-slate-900/40 border border-slate-800/60 rounded-full shadow-2xl">
            ...
            {isTimerRunning && (
              <div className="absolute inset-4 rounded-full border border-slate-800/40 animate-spin [animation-duration:60s]" />
            )}
            ...
            <div className="flex flex-col items-center justify-center text-center relative z-10">
              ...
              {isTimerRunning ? (
                ...
                <span className="text-[10px] font-mono text-amber-400 mt-4 uppercase tracking-widest font-black flex items-center space-x-1.5 bg-amber-400/5 px-2.5 py-1 border border-amber-400/20 rounded-full animate-pulse">
          ...
          <div className="flex items-center gap-6">
            <button
              onClick={() => {
                // Initialize context on play click safely
                setIsTimerRunning(prev => !prev);
              }}
              className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${
                isTimerRunning 
                  ? 'bg-amber-500 hover:bg-amber-600 shadow-lg shadow-amber-500/20 text-slate-950' 
                  : 'bg-brand-blue hover:bg-blue-600 shadow-lg shadow-brand-blue/30 text-white'
              }`}
      ...
      <footer className="border-t border-slate-900 pt-6 mt-4 w-full max-w-6xl mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex bg-slate-900 p-1 border border-slate-800 rounded">
            {(['rain', 'drone', 'alpha', 'wind'] as const).map(type => (
              <button
                key={type}
                onClick={() => handleSoundscapeToggle(type)}
                className={`px-3 py-1.5 font-mono text-[9px] uppercase tracking-widest rounded transition-all font-bold ${
                  soundType === type && isSoundPlaying
                    ? 'bg-cyan-500 text-slate-950 shadow'
                    : soundType === type
                    ? 'border border-cyan-500/30 text-cyan-400 bg-cyan-400/5'
                    : 'text-slate-400 hover:text-slate-100'
                }`}
                id={`soundscape-toggle-${type}`}
              >
                {type === 'rain' ? '🌧️ Rain' : type === 'drone' ? '🛸 Cosmic' : type === 'alpha' ? '🧠 Alpha' : '🌲 Wind'}
              </button>
            ))}
          </div>
        </div>
