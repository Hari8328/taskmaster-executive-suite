...
interface FocusSession {
  ...
  taskTitle: string;
  taskCategory: string;
  timerMode: 'pomodoro' | 'short' | 'long' | 'stopwatch';
}

const DEFAULT_SESSIONS: FocusSession[] = [
  {
    id: 'sample-session-1',
    startTime: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000 - 4 * 60 * 60 * 1000).toISOString(), // 3 days ago
    endTime: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000 - 3 * 60 * 60 * 1000 - 35 * 60 * 1000).toISOString(),
  ...
  {
    id: 'sample-session-2',
    startTime: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000 - 6 * 60 * 60 * 1000).toISOString(), // 2 days ago
    endTime: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000 - 5 * 60 * 60 * 1000).toISOString(),
  ...
  },
  {
    ...
    endTime: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000 - 1 * 60 * 60 * 1000 - 35 * 60 * 1000).toISOString(),
    durationSeconds: 1500, // 25m
  ...
  {
    id: 'sample-session-4',
    startTime: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000 - 5 * 60 * 60 * 1000).toISOString(), // Yesterday
    endTime: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000 - 4 * 60 * 60 * 1000 - 15 * 60 * 1000).toISOString(),
  ...
  {
    id: 'sample-session-5',
    startTime: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(), // Today
...
export default function FocusLogsView() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      ...
      // Seed defaults to provide immediate visual tracking for clarity
  ...
  // Available categories & modes for filters
  ...
  const filteredSessions = sessions.filter(s => {
    const matchesSearch = s.taskTitle?.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          s.taskCategory?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  // High-concentration metric
  const highlyProductiveGroup = sessions.reduce((acc: any, s) => {
  ...
  // Trend data mapping for chart: last 7 days aggregation
  const getTrendDataForChart = () => {
    ...
    sessions.forEach(s => {
      ...
      if (datesMap[sessionDateLabel] !== undefined) {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-10 max-w-7xl mx-auto space-y-10 text-left" id="focus-logs-view-root">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 bg-editorial-muted/5 border border-editorial-border/30 rounded-3xl p-6 shadow-inner">
          ...
          <div className="h-64 sm:h-80 w-full" id="focus-trend-chart-container">
            {totalFocusSeconds === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis 
                    ...
                    tickLine={false}
                    axisLine={false}
                    label={{ value: 'Minutes', angle: -90, position: 'insideLeft', offset: 10, style: { fontSize: 10, fontFamily: 'monospace', fill: '#94a3b8' } }}
                  />
                  <Tooltip 
                    contentStyle={{ 
      ...
      <div className="space-y-6">
        ...
        {filteredSessions.length === 0 ? (
          <div className="bg-slate-50 border border-slate-100/50 rounded-3xl p-12 text-center max-w-xl mx-auto space-y-4">
          ...
          <div className="relative pl-6 md:pl-10 border-l border-slate-200 mt-4 space-y-10">
            ...
            {filteredSessions.map((session, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute -left-[31px] md:-left-[47px] top-1 w-6 h-6 rounded-full flex items-center justify-center border-2 transition-all shadow-sm ${
                    isPomodoro ? 'bg-red-50 border-red-500 text-red-500' :
                    isBreak ? 'bg-emerald-50 border-emerald-500 text-emerald-500' :
                    ...
                    <div className="space-y-2 flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2.5">
                        ...
                        <span className={`text-[9px] font-mono uppercase font-bold py-0.5 px-2 rounded-full ${
                          session.timerMode === 'pomodoro' ? 'bg-red-500/10 text-red-600 border border-red-200/50' :
                          session.timerMode === 'short' ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-200/50' :
                          session.timerMode === 'long' ? 'bg-teal-500/10 text-teal-600 border border-teal-200/50' :
                          'bg-sky-500/10 text-sky-600 border border-sky-200/50'
                        }`}>
                          {session.timerMode === 'pomodoro' ? '🍅 Pomodoro Block' :
                           session.timerMode === 'short' ? '☕ Short Break' :
                           session.timerMode === 'long' ? '🌲 Long Break' :
                           '⏱️ Stopwatch Tracker'}
                        </span>
