...
const Header = ({ currentView, onViewChange, onNewTask, onProfileOpen, onHelp }) => {
  ...
  const getMatchedItems = () => {
    const list = [];
    const allShortcuts = [
      { key: "D", name: "Go to Dashboard", description: "Navigate to the primary overview board", icon: Compass, action: () => {
        onViewChange?.("dashboard");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "P", name: "Go to Projects Board", description: "View active projects and boards", icon: Navigation, action: () => {
        onViewChange?.("projects");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "T", name: "Go to Tasks Repository", description: "Complete backlog checklist catalog", icon: CheckSquare, action: () => {
        onViewChange?.("tasks");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "I", name: "Go to Inspiration Wall", description: "Daily quotes, advice, and growth records", icon: Sparkles, action: () => {
        onViewChange?.("inspiration");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "G", name: "Go to AI Suggestions", description: "Smart assistant prompt categories", icon: Sparkles, action: () => {
        onViewChange?.("suggestions");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "R", name: "Go to Project Roadmap", description: "Phases, streams, and timeline progress", icon: Clock, action: () => {
        onViewChange?.("roadmap");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "A", name: "Open AI Mind Coach", description: "Live mentoring and task synthesis chat", icon: UserIcon, action: () => {
        onViewChange?.("chat");
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "W", name: "Open Weekly Digest", description: "Performance graphs and metrics", icon: Keyboard, action: () => {
        window.dispatchEvent(new CustomEvent("open-weekly-digest"));
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "C", name: "Create New Task", description: "Open task creator modal form", icon: ArrowRight, action: () => {
        onNewTask?.();
        setIsSearchFocused(false);
        setSearchQuery("");
      } },
      { key: "K", name: "Terminal Cheat Sheet", description: "Toggle floating hotkey help box", icon: Keyboard, action: () => {
        window.dispatchEvent(new KeyboardEvent("keydown", { key: "k" }));
        setIsSearchFocused(false);
        setSearchQuery("");
      } }
    ];
    const matchedShortcuts = allShortcuts.filter(
      (s) => s.name.toLowerCase().includes(searchQuery.toLowerCase()) || s.description.toLowerCase().includes(searchQuery.toLowerCase()) || s.key.toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    const matchedProfile = searchQuery.trim() === "" ? [] : profileDetails.filter(
      (p) => p.label.toLowerCase().includes(searchQuery.toLowerCase()) || p.value.toLowerCase().includes(searchQuery.toLowerCase()) || "profile".includes(searchQuery.toLowerCase()) || "details".includes(searchQuery.toLowerCase()) || "user".includes(searchQuery.toLowerCase())
    );
    ...
    const matchedTasks = searchQuery.trim() === "" ? [] : allTasksForSearch.filter(
      (t) => t.title?.toLowerCase().includes(searchQuery.toLowerCase()) || t.description?.toLowerCase().includes(searchQuery.toLowerCase()) || t.category?.toLowerCase().includes(searchQuery.toLowerCase())
    ).slice(0, 4);
  ...
  return <header className="h-16 md:h-24 px-3 sm:px-6 md:px-10 flex items-center justify-between sticky top-0 z-40 bg-editorial-paper/80 backdrop-blur-md border-b border-editorial-border transition-colors duration-300">
  ...
  >
    ...
    return <motion.div
      ...
      className="fixed left-4 right-4 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:w-[320px] md:w-[400px] lg:w-[480px] mt-2 bg-zinc-950 text-zinc-100 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.85)] border border-zinc-800/80 overflow-hidden z-[60] p-4 max-h-[80vh] overflow-y-auto custom-scrollbar text-left"
    >
                  {
      ...
      return matchedShortcuts.map((s) => {
        ...
        return <button
          ...
          className={cn(
            ...
            isHighlighted ? "bg-zinc-900 border-zinc-700/80 shadow-lg shadow-black/40 translate-x-1" : "bg-transparent border-transparent hover:bg-zinc-900/40 hover:border-zinc-900/60"
          )}
        >
                            <div className="flex items-center space-x-3">
                              <div className={cn(
          "p-1.5 rounded-lg transition-all",
          isHighlighted ? "bg-sky-500/15 text-sky-400" : "bg-zinc-900 text-zinc-400 group-hover/item:text-sky-400 group-hover/item:bg-sky-500/10"
        )}>
          ...
          isHighlighted ? "bg-sky-500 border-sky-400 text-zinc-950 font-bold" : "bg-zinc-900 border-zinc-800 text-zinc-550 group-hover/item:bg-zinc-850 group-hover/item:text-zinc-300"
        )}>
                              {s.key}
                            </kbd>
                          </button>;
      ...
      const isHighlighted = absIdx === highlightedIndex;
      return <div className="mt-3 pt-3 border-t border-zinc-800/60 mb-3">
                        ...
                        <div className="space-y-1.5">
                          {profileItem.profileDetail && profileItem.profileDetail.map((detail, index) => <div key={index} className="px-2.5 py-2 bg-zinc-900/40 border border-zinc-800/40 rounded-xl flex flex-col">
        ...
        className={cn(
          ...
          isHighlighted ? "bg-sky-500 border-sky-400 text-zinc-950 shadow-md shadow-sky-500/15 translate-x-1 font-bold" : "border-zinc-800 text-zinc-300 hover:bg-zinc-900 hover:text-white"
        )}
      >
      ...
      if (matchedTasks.length === 0) {
        if (searchQuery.trim() !== "") {
          return <div className="mt-3 pt-3 border-t border-zinc-800/60">
      ...
      return <div className="mt-3 pt-3 border-t border-zinc-800/60">
        ...
        return <button
          ...
          className={cn(
            ...
            isHighlighted ? "bg-zinc-900 border-zinc-700/80 shadow-lg translate-x-1" : "bg-zinc-900/30 hover:bg-zinc-900/60 border-zinc-900 hover:border-zinc-800"
          )}
        >
          ...
          t.taskStatus?.toLowerCase() === "completed" ? "bg-emerald-500/10 text-emerald-400" : t.taskStatus?.toLowerCase() === "in_progress" ? "bg-cyan-500/10 text-cyan-400" : "bg-amber-500/10 text-amber-400"
        )}>
  ...
  >
            ...
            {unreadCount > 0 && <span className="absolute top-2.5 right-2.5 w-4 h-4 bg-editorial-accent text-[10px] font-bold text-white flex items-center justify-center rounded-full border-2 border-editorial-paper z-20">
  ...
  >
    ...
    className="fixed left-4 right-4 top-16 sm:absolute sm:left-auto sm:right-0 sm:top-full sm:w-96 mt-2 max-w-md bg-zinc-950 text-zinc-100 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.85)] border border-zinc-800/80 overflow-hidden z-50 py-2 text-left animate-in fade-in slide-in-from-top-2 duration-200"
  >
    ...
    permissionStatus === "granted" ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : permissionStatus === "denied" ? "bg-red-500" : "bg-yellow-500"
  )} />
                       <span className="text-[10px] font-mono text-zinc-500">
                         {permissionStatus === "granted" ? "Browser Push Active" : permissionStatus === "denied" ? "Push Blocked" : "Push Not Configured"}
                       </span>
                    </div>
                  </div>
  ...
  >
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  {safeNotifications.length === 0 ? <div className="px-10 py-12 text-center">
                    ...
                    </div> : safeNotifications.map((n) => <div
    key={n.id}
  ...
  >
    ...
    n.read ? "text-zinc-400" : "text-zinc-100 font-medium"
                        ...
                        {!n.read && <div className="w-2 h-2 bg-sky-400 rounded-full mt-2 shrink-0 shadow-sm shadow-sky-400/40" />}
                      </div>)}
                </div>
              </motion.div>}
          </AnimatePresence>
