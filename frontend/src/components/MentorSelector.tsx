...
interface MentorSelectorProps {
  className?: string;
  onChanged?: (mentorId: string) => void;
}

export default function MentorSelector({ className, onChanged }: MentorSelectorProps) {
  const [currentMentor, setCurrentMentor] = useState<InspirationMentor>(() => getSelectedMentor());
  ...
  // Poll or listen for external mentor changes
  useEffect(() => {
  ...
  return (
    <div className={cn("space-y-4", className)} id="inspiration-mentor-selector-container">
      ...
      <div className="border border-editorial-border bg-editorial-paper p-6 relative overflow-hidden transition-all duration-300 group hover:shadow-lg rounded-xl flex flex-col md:flex-row gap-6 md:items-center">
        ...
        <div className="relative shrink-0 flex items-center justify-center">
          <div className="w-20 h-20 rounded-full border-2 border-editorial-ink overflow-hidden shadow-md bg-editorial-muted/10 flex items-center justify-center">
            {imgErrors[currentMentor.id] ? (
              <span className="font-mono font-bold text-xl text-editorial-ink">
      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-editorial-ink/65 backdrop-blur-sm">
            <motion.div
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isSelected 
                            ? "border-editorial-ink bg-editorial-muted/5 shadow-inner" 
                            : "border-editorial-border hover:border-editorial-ink bg-editorial-paper"
                        )}
                      >
                        <div className="space-y-4">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {mentor.jerseyNumber && (
                                    <span className="font-mono text-[9px] font-bold px-1.5 py-0.5 bg-amber-500/10 text-amber-600 rounded border border-amber-500/20">
                                      #{mentor.jerseyNumber}
                                    </span>
                                  )}
                                </div>
                            ...
                            {isSelected ? (
                              <span className="px-2 py-1 bg-green-50 border border-green-200 text-green-700 font-mono text-[9px] uppercase font-bold rounded">
                                Active Mentor
                              </span>
                            ) : (
                              <span className="text-[10px] font-mono text-editorial-muted group-hover:text-editorial-ink transition-colors uppercase font-bold">
                                Select &bull; &rarr;
                              </span>
                            )}
                          </div>
                        ...
                        <div className="pt-4 mt-2">
                          <button
                            ...
                            className={cn(
                              ...
                              isSelected 
                                ? "bg-editorial-ink hover:opacity-90 text-editorial-paper border-editorial-ink" 
                                : "bg-transparent text-editorial-ink hover:bg-editorial-ink hover:text-white border-editorial-border hover:border-editorial-ink"
                            )}
                          >
