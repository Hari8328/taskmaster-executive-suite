...
import { 
  X, User as UserIcon, Mail, Image as ImageIcon, AlignLeft, Save, 
  Loader2, Phone, Settings, LogOut, Volume2, VolumeX, Sun, Moon, Eye, HelpCircle 
} from 'lucide-react';
...
interface ProfileModalProps {
  ...
  onThemeChange?: (theme: 'light' | 'dark' | 'eye') => void;
}
...
const ProfileModal: React.FC<ProfileModalProps> = ({ 
  ...
  // Notification Alert Sound State
  ...
  useEffect(() => {
    if (isOpen) {
      fetchProfile();
      // Keep sound settings state synchronized in case they changed in header
      const saved = localStorage.getItem('notification_sound_enabled');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Dispatch a custom event to notify other mounted components (like the Dashboard)
              ...
              <form onSubmit={handleSave} className="space-y-6">
                <div className="flex justify-center mb-6">
                  <div className="relative group">
                    <div className="w-20 h-20 rounded-full border-2 border-editorial-border overflow-hidden bg-editorial-muted/10 flex items-center justify-center">
                      {avatarUrl ? (
                        <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                      ) : (
                        <UserIcon size={28} className="text-editorial-muted" />
                      )}
                    </div>
                  </div>
                </div>
                      ...
                      {[
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            theme === t.id 
                              ? "bg-white border-editorial-ink text-editorial-ink shadow-sm" 
                              : "border-editorial-border/40 text-editorial-muted hover:text-editorial-ink hover:bg-white/40"
                          )}
                        >
