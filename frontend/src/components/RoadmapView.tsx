...
const RoadmapView: React.FC = () => {
  ...
  const [startTime, setStartTime] = useState<string>(() => localStorage.getItem('roadmap_start_time') || "09:00");
  const [endTime, setEndTime] = useState<string>(() => localStorage.getItem('roadmap_end_time') || "18:00");
  ...
  const fetchRoadmap = async (isRefetch = false, customStart = startTime, customEnd = endTime) => {
    ...
    try {
      ...
      // Calculate a dynamic distributed fallback based on chosen start and end hours
      const fallbackTimes = (() => {
        try {
  ...
  return (
    <main className="p-4 md:p-8 max-w-5xl mx-auto space-y-8 md:space-y-12 overflow-y-auto h-full pb-24 custom-scrollbar">
