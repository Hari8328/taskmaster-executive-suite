...
const SuggestionsView = () => {
  ...
  const getCategoryIcon = (category) => {
    switch (category) {
      case "Focus":
        return <Target size={20} className="text-brand-blue" />;
      case "Organization":
        return <Layers size={20} className="text-indigo-500" />;
      case "Mindset":
        return <Brain size={20} className="text-editorial-accent" />;
      default:
  ...
  const getCategoryBg = (category) => {
    switch (category) {
      case "Focus":
        return "bg-brand-blue/5 border-brand-blue/20";
      case "Organization":
        return "bg-indigo-500/5 border-indigo-500/20";
      case "Mindset":
        return "bg-editorial-accent/5 border-editorial-accent/20";
      default:
        return "bg-editorial-muted/5 border-editorial-border/20";
    }
  };
  if (loading) {
  ...
  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
        {suggestions.map((suggestion, index) => <motion.div
    key={index}
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay: index * 0.1 }}
    className={cn(
      "p-6 md:p-8 border rounded-sm flex flex-col justify-between group hover:shadow-md transition-shadow duration-300",
  ...
  >
            ...
            <div className="space-y-3">
                <h3 className="text-xl md:text-2xl font-serif italic text-editorial-ink transition-colors duration-300 group-hover:text-brand-blue">
                  {suggestion.title}
                </h3>
