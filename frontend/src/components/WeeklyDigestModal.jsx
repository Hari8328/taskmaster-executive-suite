...
export default function WeeklyDigestModal({ isOpen, onClose, tasks, stats }) {
...
${d.getDate()}`;
      ...
      data.push({
        ...
        focusMinutes: Math.round(dayFocusMs / (1e3 * 60))
      });
    }
    return data;
  }, [completedTasks]);
  ...
  const completedSvgRef = useRef(null);
  const focusSvgRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const svg = d3.select(completedSvgRef.current).attr("width", "100%").attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).style("overflow", "visible");
    svg.selectAll("*").remove();
    const xScale = d3.scaleBand().domain(data.map((d) => d.dateLabel)).range([margin.left, width - margin.right]).padding(0.35);
    const maxCount = d3.max(data, (d) => d.completedCount) || 0;
    const yScale = d3.scaleLinear().domain([0, Math.max(4, maxCount + 1)]).range([height - margin.bottom, margin.top]);
    svg.append("g").attr("class", "opacity-10").selectAll("line").data(yScale.ticks(4)).enter().append("line").attr("x1", margin.left).attr("x2", width - margin.right).attr("y1", (d) => yScale(d)).attr("y2", (d) => yScale(d)).attr("stroke", "#000000").attr("stroke-width", 1).attr("stroke-dasharray", "2,2");
    const xAxis = d3.axisBottom(xScale).tickSize(3);
    const yAxis = d3.axisLeft(yScale).ticks(3).tickSize(3).tickFormat(d3.format("d"));
    svg.append("g").attr("transform", `translate(0, ${height - margin.bottom})`).call(xAxis).attr("class", "text-[10px] font-mono text-editorial-muted").select(".domain").attr("stroke", "#e2e8f0");
    svg.append("g").attr("transform", `translate(${margin.left}, 0)`).call(yAxis).attr("class", "text-[10px] font-mono text-editorial-muted").select(".domain").attr("stroke", "#e2e8f0");
    svg.selectAll(".bar").data(data).enter().append("rect").attr("class", "fill-editorial-ink/90 hover:fill-editorial-ink transition-colors cursor-pointer").attr("x", (d) => xScale(d.dateLabel) || 0).attr("y", (d) => yScale(d.completedCount)).attr("width", xScale.bandwidth()).attr("height", (d) => height - margin.bottom - yScale(d.completedCount)).attr("rx", 2);
    svg.selectAll(".bar-label").data(data).enter().append("text").attr("x", (d) => (xScale(d.dateLabel) || 0) + xScale.bandwidth() / 2).attr("y", (d) => yScale(d.completedCount) - 5).attr("text-anchor", "middle").attr("class", "text-[9px] font-mono font-bold fill-editorial-ink").text((d) => d.completedCount > 0 ? d.completedCount : "0");
  ...
  useEffect(() => {
    ...
    const svg = d3.select(focusSvgRef.current).attr("width", "100%").attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).style("overflow", "visible");
    svg.selectAll("*").remove();
    const xScale = d3.scalePoint().domain(data.map((d) => d.dateLabel)).range([margin.left, width - margin.right]);
    const maxFocus = d3.max(data, (d) => d.focusMinutes) || 0;
    const yScale = d3.scaleLinear().domain([0, Math.max(30, maxFocus + 10)]).range([height - margin.bottom, margin.top]);
    svg.append("g").attr("class", "opacity-10").selectAll("line").data(yScale.ticks(4)).enter().append("line").attr("x1", margin.left).attr("x2", width - margin.right).attr("y1", (d) => yScale(d)).attr("y2", (d) => yScale(d)).attr("stroke", "#000000").attr("stroke-width", 1).attr("stroke-dasharray", "2,2");
    const xAxis = d3.axisBottom(xScale).tickSize(3);
    ...
    const lineGen = d3.line().x((d) => xScale(d.dateLabel) || 0).y((d) => yScale(d.focusMinutes)).curve(d3.curveMonotoneX);
    const areaGen = d3.area().x((d) => xScale(d.dateLabel) || 0).y0(height - margin.bottom).y1((d) => yScale(d.focusMinutes)).curve(d3.curveMonotoneX);
    svg.append("path").datum(data).attr("fill", `url(#${gradientId})`).attr("d", areaGen);
    svg.append("path").datum(data).attr("fill", "none").attr("stroke", "#3b82f6").attr("stroke-width", 2.5).attr("d", lineGen);
    svg.selectAll(".dot").data(data).enter().append("circle").attr("cx", (d) => xScale(d.dateLabel) || 0).attr("cy", (d) => yScale(d.focusMinutes)).attr("r", 4.5).attr("class", "fill-white stroke-brand-blue stroke-[2px] transition-transform duration-200 cursor-pointer hover:scale-130");
    svg.selectAll(".focus-value-label").data(data).enter().append("text").attr("x", (d) => xScale(d.dateLabel) || 0).attr("y", (d) => yScale(d.focusMinutes) - 7).attr("text-anchor", "middle").attr("class", "text-[8.5px] font-mono font-bold fill-brand-blue").text((d) => d.focusMinutes > 0 ? `${d.focusMinutes}m` : "");
  }, [last7DaysData, isOpen]);
  useEffect(() => {
    if (!pieSvgRef.current || !isOpen || categoryData.length === 0) return;
    const data = categoryData.slice(0, 5);
    const width = 160;
    const height = 160;
    const radius = Math.min(width, height) / 2;
    const svg = d3.select(pieSvgRef.current).attr("width", width).attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).style("overflow", "visible");
    svg.selectAll("*").remove();
    const g = svg.append("g").attr("transform", `translate(${width / 2}, ${height / 2})`);
    const color = d3.scaleOrdinal().domain(data.map((d) => d.category)).range(["#1e293b", "#2563eb", "#10b981", "#f59e0b", "#ec4899", "#64748b"]);
    const pie = d3.pie().value((d) => d.count).sort(null);
    const path = d3.arc().outerRadius(radius - 8).innerRadius(radius - 24);
    const arcOver = d3.arc().outerRadius(radius - 4).innerRadius(radius - 24);
    const arcs = g.selectAll(".arc").data(pie(data)).enter().append("g").attr("class", "arc");
    arcs.append("path").attr("d", path).attr("fill", (d) => color(d.data.category)).attr("stroke", "#ffffff").attr("stroke-width", 2).attr("class", "transition-all duration-300 cursor-pointer").on("mouseenter", function() {
      d3.select(this).transition().duration(150).attr("d", arcOver);
    }).on("mouseleave", function() {
      d3.select(this).transition().duration(150).attr("d", path);
    });
    const grandTotal = d3.sum(categoryData, (d) => d.count);
    g.append("text").attr("text-anchor", "middle").attr("dy", "-3px").attr("class", "text-[8.5px] font-mono uppercase fill-slate-400 font-semibold tracking-wider").text("Archives");
    g.append("text").attr("text-anchor", "middle").attr("dy", "14px").attr("class", "text-xl font-serif font-serif italic font-bold fill-editorial-ink").text(grandTotal);
  }, [categoryData, isOpen]);
  ...
  >
    ...
    /* 2. Total focus time logged */
                ...
                <div className="border border-editorial-border p-4 flex flex-col justify-between">
                  ...
                  <div className="mt-2 flex items-baseline justify-between">
                    <span className="text-3xl font-serif font-bold text-editorial-ink">
                      {weeklySummary.totalFocusMinutes > 60 ? `${Math.floor(weeklySummary.totalFocusMinutes / 60)}h ${weeklySummary.totalFocusMinutes % 60}m` : `${weeklySummary.totalFocusMinutes}m`}
                    </span>
    ...
    /* 4. Weekly growth Index */
                ...
                <div className={`border p-4 flex flex-col justify-between transition-colors ${weeklySummary.growthPct >= 0 ? "border-emerald-200 bg-emerald-50/25" : "border-orange-200 bg-orange-50/25"}`}>
                  <span className="font-mono text-[10px] uppercase text-editorial-muted tracking-wider block">Growth Index</span>
                  <div className="mt-2 flex items-baseline justify-between">
                    <div className="flex items-center space-x-1.5">
                      <span className={`text-3xl font-serif font-black ${weeklySummary.growthPct >= 0 ? "text-emerald-700" : "text-orange-700"}`}>
                        {weeklySummary.growthPct >= 0 ? "+" : ""}{weeklySummary.growthPct}%
                      </span>
                      {weeklySummary.growthPct >= 0 ? <TrendingUp size={16} className="text-emerald-600" /> : <TrendingDown size={16} className="text-orange-600" />}
                    </div>
    ...
    /* Chart 1 container: Completed Tasks */
                ...
                <div className="border border-editorial-border bg-white p-5 space-y-4 rounded-sm flex flex-col justify-between">
                  ...
                  <div className="w-full h-48 flex items-end justify-center relative pl-2 pt-2">
                    {weeklySummary.totalCompletions === 0 && <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/5 backdrop-blur-[1px] p-4 text-center z-10 rounded">
    ...
    /* Chart 2 container: Focus Duration */
                ...
                <div className="border border-editorial-border bg-white p-5 space-y-4 rounded-sm flex flex-col justify-between">
                  ...
                  <div className="w-full h-48 flex items-end justify-center relative pl-2 pt-2">
                    {weeklySummary.totalFocusMinutes === 0 && <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/5 backdrop-blur-[1px] p-4 text-center z-10 rounded">
    ...
    /* Category Donut chart and breakdown list */
                ...
                <div className="md:col-span-5 border border-editorial-border p-5 rounded-sm flex flex-col justify-between bg-white">
                  ...
                  {categoryData.length > 0 ? <div className="flex flex-col sm:flex-row items-center justify-around gap-6 mt-4">
    ...
    return <div key={item.category} className="flex items-center justify-between text-xs font-mono">
                    ...
                    </div> : <div className="py-12 text-center text-xs text-slate-400 font-serif italic">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <h4 className="text-lg font-serif italic font-bold leading-snug">
                        {weeklySummary.growthPct >= 0 ? "You are expanding your cognitive throughput velocity!" : "Your cognitive work buffer requires tuning."}
                      </h4>
                      <p className="text-xs text-editorial-paper/85 leading-relaxed font-serif italic">
                        {weeklySummary.growthPct >= 0 ? `With a growth rate of +${weeklySummary.growthPct}% in completions, your consistency score indicates high mental endurance. Your structured execution, paired with immersive Deep Focus, has neutralized potential attention fatigue.` : `Completions fell slightly back by ${Math.abs(weeklySummary.growthPct)}% vs the prior 7-day period. This is completely standard during recovery phases. Consider leveraging shorter pomodoro intervals (25-minute sprints with guided box breathing) to smoothly restart momentum.`}
                      </p>
                    </div>
                  </div>
