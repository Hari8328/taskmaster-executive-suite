export const playReminderSound = () => {
  ...
  try {
    ...
    gain2.gain.exponentialRampToValueAtTime(1e-3, now + 0.6);
    osc2.connect(gain2);
...
export const playCompleteSound = () => {
  ...
  try {
    ...
    notes.forEach((freq, idx) => {
      ...
      gain.gain.exponentialRampToValueAtTime(1e-3, now + delays[idx] + durations[idx]);
      osc.connect(gain);
...
export const saveCompletedTaskTime = (durationMs) => {
...
export const checkAndUnlockAchievements = (allTasks) => {
  ...
  const hasTrackedDuration = completedTasks.some((t) => t.startedAt && t.completedAt && new Date(t.completedAt).getTime() - new Date(t.startedAt).getTime() > 1e3);
  const newUnlocks = [];
  const updatedAchievements = achievements.map((ach) => {
    ...
    switch (ach.id) {
      ...
      case "master_architect":
        ...
        break;
      case "deep_focus":
        shouldUnlock = totalFocusTimeMs >= 15 * 60 * 1e3;
        break;
      case "speedy_mason":
...
export const dispatchCelebration = (payload) => {
